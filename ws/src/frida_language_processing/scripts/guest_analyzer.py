...
class GuestAnalyzer:
    ...
    def guest_info_requested(self, request: GuestInfo) -> GuestInfoResponse:
        ...
        guest_name = text_completion.split(
            ",")[0].strip().capitalize()  # Extract the guest name
        guest_drink = text_completion.split(",")[1].strip(
            ' .').lower()  # Extract the guest favorite drink
    ...
    def analysis_callback(self, goal: GuestAnalysisGoal) -> None:
        ...
        picture = rospy.wait_for_message(
            CAMERA_TOPIC, Image, timeout=10.0)  # Wait for the picture
