...
rp = rospkg.RosPack()
DATAFRAMES_DIR = rp.get_path(
    'frida_language_processing') + "/scripts/dataframes/"
...
class ItemCategorization:
    ...
    def __init__(self) -> None:
        ...
        rospy.Service(ITEMS_EXTRACT_TOPIC, ItemsCategory,
                      self.get_local_embedding)
    ...
    def extract_item(self, req):
        ...
        possible_items = self.get_similar_items(
            item_embedding, self.items_dataframe)
    ...
    def extract_category(self, req):
        ...
        for item in items:
            ...
            item_category = self.get_category_similarity(
                item_embedding, self.items_dataframe)
    ...
    def local_extract_category(self, req):
        ...
        for category_id, category_data in self.category_embeddings.items():
            if category_data is not None:
                category_data['similarity'] = self.get_embeddings_similarity(
                    item_embedding, category_data['category_embedding'])
    ...
    def get_category_similarity(self, input, dataframe):
        ...
        dataframe['similarity'] = dataframe['category_embedding'].apply(
            lambda x: np.dot(x, input))
        sorted_categories = dataframe.sort_values(
